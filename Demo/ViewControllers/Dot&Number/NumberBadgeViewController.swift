//
//  NumberBadgeViewController.swift
...
class NumberBadgeViewController: UIViewController {
    ...
    var dataSource: [(CustomSegmentTitleDataSource, CustomSegmentChildViewControllerDataSource, CustomSegmentBadgeDataSource)] = [
        (.title("推荐"), .tableChild, .badge(0)), (.title("小视频"), .buttonChild, .badge(10)),
        (.title("榜单"), .collectionChild, .badge(9)), (.title("超级IP季"), .child, .badge(0)),
        (.title("百年影像"), .collectionChild, .badge(100)), (.title("播单"), .child, .badge(10)),
        (.title("二次元"), .tableChild, .badge(1000)), (.title("涨姿势"), .buttonChild, .badge(0))
    ]
    ...
    private func constructView() {
        ...
        titleStyle.edgeInsets = UIEdgeInsets(top: 15.0, left: 10.0, bottom: -5.0, right: -10.0)
        ...
        titleContainerStyle.badgeBorderColor = UIColor.clear.cgColor
    ...
    private func constructNavigationBar() {
        self.navigationItem.title = "Number Badge"
...
extension NumberBadgeViewController: JSSegmentControlDataSource {
...
extension NumberBadgeViewController: JSSegmentControlDelegate {
