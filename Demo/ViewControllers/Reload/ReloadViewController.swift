...
class ReloadViewController: UIViewController {
    ...
    private func constructNavigationBar() {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .refresh, target: self, action: #selector(refreshBarButtonItemTrigger(_:)))
    ...
    // MARK:
    @objc private func refreshBarButtonItemTrigger(_ barButtonItem: UIBarButtonItem) {
        self.dataSource = self.dataSource.reversed()
        self.segmentControl?.reload()
    }
}

extension ReloadViewController: JSSegmentControlDataSource {
...
extension ReloadViewController: JSSegmentControlDelegate {
