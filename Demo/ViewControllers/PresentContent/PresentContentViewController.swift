...
class PresentContentViewController: UIViewController {
    ...
    var dataSource: [(CustomSegmentTitleDataSource, CustomSegmentChildViewControllerDataSource)] = [
        (.title("通知"), .tableChild), (.title("私信"), .collectionChild)
    ]
...
extension PresentContentViewController: JSSegmentControlDataSource {
...
extension PresentContentViewController: JSSegmentControlDelegate {
    ...
    func segmentControl(_ segmentControl: JSSegmentControl, willAppear controller: UIViewController, forItemAt index: Int) {
        print("isPresentController \(controller == segmentControl.presentContent)")
